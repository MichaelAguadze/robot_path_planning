function new_gen = create_new_gen(mmx,bit_count,point_ls)
    ...
    % Generate a lookup table to integer positions into binary
...
function [child1,child2] = path_genetic_cross(in_mmx, bit_count, look_up, loc_bunpack)
    ...
    % changed
    [p1,p3] = preserve_pos(loc_bunpack, look_up, bit_count);
...
% end
% In create_new_gen.m, modify the preserve_pos function (around line 94):
function [p1,p3] = preserve_pos(loc_bunpack, look_up, bit_count_for_conversion) % Added bit_count_for_conversion
    % Force starting position
    p1_dec = loc_bunpack(1,1);
    p1 = convert_to_bin(p1_dec, bit_count_for_conversion); % Pass bit_count
    % Force ending position
    p3_dec = loc_bunpack(1,2);
    p3 = convert_to_bin(p3_dec, bit_count_for_conversion); % Pass bit_count
end
% ------------------------------------------------------------------------
...
%HARDCODED, but must have a way to generate
    for ii = 1:bx
        ...
        % toolbox
        bin_arr = dec2bin(var_dec,bit_count);
...
%     end
% end
% In create_new_gen.m, replace the old convert_to_bin function with this:
function [bit_out] = convert_to_bin(dec_in, bit_count_param) % look_up argument was unused
    bit_out = zeros(1, bit_count_param);
    temp_bin_str = dec2bin(dec_in, bit_count_param);

    % Ensure temp_bin_str has bit_count_param digits, padding with leading zeros if necessary
    % dec2bin should handle this, but as a safeguard for very small dec_in:
    if length(temp_bin_str) < bit_count_param
        temp_bin_str = sprintf('%0*s', bit_count_param, temp_bin_str);
    end

    for i = 1:bit_count_param
        bit_out(i) = str2double(temp_bin_str(i));
    end
end
